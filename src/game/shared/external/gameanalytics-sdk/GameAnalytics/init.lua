...
local function ErrorHandler(message, trace, scriptName, player)
	...
	if player then
		...
		m = string.gsub(m, player.Name, "[LocalPlayer]") -- so we don't flood the same errors with different player names
	end
...
local function WarningHandler(message, player)
	local messageTmp = "(null)"
	if message ~= nil then
		messageTmp = message
	end

	local m = (if player then "Client" else "Server") .. " warning: " .. messageTmp
	if #m > 8192 then
		m = string.sub(m, 1, 8192)
	end

	-- Pull out names so we don't flood the same errors with different players
	local userId = nil
	if player then
		userId = player.UserId
		m = string.gsub(m, player.Name, "[LocalPlayer]")
	else
		for _, plr in pairs(Players:GetPlayers()) do
			m = string.gsub(m, plr.Name, "[PlayerName]")
		end
	end
...
local function WarningHandlerFromClient(message, player)
	if state.ReportErrors then
		return WarningHandler(message, player)
	end
end
...
ReplicatedStorage.GameAnalyticsError.OnServerEvent:Connect(function(player, message, trace, scriptName)
	if trace == "Warning" then
		WarningHandlerFromClient(message, player)
	else
